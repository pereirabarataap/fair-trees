...
# New: Biased classification criterion with Z support
cdef class BiasedClassificationCriterion(ClassificationCriterion):
    """Classification criterion with bias regularization via Z."""
    
    # Z-related fields
    cdef const float64_t[:, ::1] z         # Values of z (categorical features for bias)
    cdef intp_t n_z_outputs                # Number of Z columns
    cdef intp_t[::1] n_z_classes           # Number of classes in each Z column
    cdef intp_t max_n_z_classes            # Maximum number of classes across Z columns
    cdef float64_t theta                   # Bias regularization strength [0,1]
    cdef int Z_agg_mode                    # 0: mean, 1: max
    
    # Z statistics (similar to Y statistics but for Z)
    cdef float64_t[:, ::1] z_sum_total     # Sum of weighted counts for Z
    cdef float64_t[:, ::1] z_sum_left      # Z stats for left child
    cdef float64_t[:, ::1] z_sum_right     # Z stats for right child
    cdef float64_t[:, ::1] z_sum_missing   # Z stats for missing values
    
    # Methods for Z impurity calculation
    ...
    # Methods for combined (Y+Z) impurity calculation
