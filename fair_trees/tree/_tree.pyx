        ...
        with nogil:
            ...
            while not builder_stack.empty():
                ...
                node_id = tree._add_node(parent, is_left, is_leaf, split.feature,
                                         ...
                                         (0.0 if is_leaf else split.improvement),
...
cdef class Tree:
    ...
    def improvement(self):
        return self._get_node_ndarray()['improvement'][:self.node_count]
    ...
    cpdef compute_feature_importances(self, normalize=True, use_auc_importances=False):
        """Computes the importance of each feature (aka variable).
        
        Parameters
        ----------
        normalize : bool, default=True
            If True, normalize the importances to sum to 1.
        use_auc_importances : bool, default=False
            If True, use the AUC-based importance calculation (weighted gain per split).
            This should be used when the tree was built with criterion="auc".
        """
        cdef Node* left
        cdef Node* right
        ...
        if use_auc_importances:
            # Use AUC-based importances: weighted gain per split
            # The gain is stored in node.improvement
            with nogil:
                while node != end_node:
                    if node.left_child != _TREE_LEAF:
                        # For AUC criterion, node.improvement contains the gain
                        importances[node.feature] += (
                            node.weighted_n_node_samples * node.improvement
                        )
                    node += 1
            
            # Normalize by total weight of root node
            for i in range(self.n_features):
                importances[i] /= nodes[0].weighted_n_node_samples
        else:
            # Standard sklearn method: weighted impurity decrease
            with nogil:
                while node != end_node:
                    if node.left_child != _TREE_LEAF:
                        # ... and node.right_child != _TREE_LEAF:
...
cdef int _cost_complexity_prune_auc(uint8_t[:] leaves_in_subtree,  # OUT
                                   Tree orig_tree,
                                   _CCPPruneController controller) except -1:
    """Gain-based cost complexity pruning for criterion='auc'.

    Uses per-node stored gain at orig_tree.nodes[node].improvement.

    Subtree gain:
        G(t) = sum_{s in internal(subtree(t))} (w_s / w_root) * gain_s

    Effective alpha:
        alpha(t) = G(t) / (n_leaves(t) - 1)

    IMPORTANT:
    - leaves_in_subtree[i] == 1 means "node i is a leaf in the PRUNED tree".
      This is what _build_pruned_tree(...) expects.
    - We also keep an internal 'in_subtree' mask meaning "node still present".
    """
    cdef intp_t n_nodes = orig_tree.node_count

    cdef cnp.ndarray[cnp.float64_t, ndim=1] r_branch_nd = np.zeros(n_nodes, dtype=np.float64)
    cdef cnp.ndarray[cnp.float64_t, ndim=1] r_node_nd = np.zeros(n_nodes, dtype=np.float64)
    cdef cnp.ndarray[cnp.intp_t, ndim=1] parent_nd = np.empty(n_nodes, dtype=np.intp)
    cdef cnp.ndarray[cnp.intp_t, ndim=1] n_leaves_nd = np.empty(n_nodes, dtype=np.intp)
    cdef cnp.ndarray[cnp.uint8_t, ndim=1] in_subtree_nd = np.ones(n_nodes, dtype=np.uint8)
    cdef cnp.ndarray[cnp.uint8_t, ndim=1] candidate_nd = np.zeros(n_nodes, dtype=np.uint8)

    cdef float64_t[:] r_branch = r_branch_nd
    cdef float64_t[:] r_node = r_node_nd
    cdef intp_t[:] parent = parent_nd
    cdef intp_t[:] n_leaves_subtree = n_leaves_nd
    cdef uint8_t[:] in_subtree = in_subtree_nd
    cdef uint8_t[:] candidate_nodes = candidate_nd

    # Extract C-level arrays from Tree before entering nogil block
    cdef intp_t[:] children_left = orig_tree.children_left
    cdef intp_t[:] children_right = orig_tree.children_right
    cdef Node* nodes = orig_tree.nodes
    cdef float64_t[:] weighted_n_node_samples = orig_tree.weighted_n_node_samples

    cdef intp_t node_idx, left, right
    cdef intp_t cur, sp, sp2
    cdef float64_t r_diff, effective_alpha, alpha_min
    cdef intp_t best_node
    cdef intp_t n_leaves, parent_idx

    # stack for postorder traversal
    cdef cnp.ndarray[cnp.intp_t, ndim=1] stack_nodes_nd = np.empty(2 * n_nodes + 1, dtype=np.intp)
    cdef cnp.ndarray[cnp.uint8_t, ndim=1] stack_state_nd = np.empty(2 * n_nodes + 1, dtype=np.uint8)
    cdef intp_t[:] stack_nodes = stack_nodes_nd
    cdef uint8_t[:] stack_state = stack_state_nd

    # stack for pruning descendants
    cdef cnp.ndarray[cnp.intp_t, ndim=1] stack2_nd = np.empty(n_nodes, dtype=np.intp)
    cdef intp_t[:] stack2 = stack2_nd

    with nogil:
        # init
        for node_idx in range(n_nodes):
            parent[node_idx] = _TREE_UNDEFINED
            left = children_left[node_idx]
            if left == _TREE_LEAF:
                # original leaf => leaf in pruned tree
                leaves_in_subtree[node_idx] = 1
                candidate_nodes[node_idx] = 0
                n_leaves_subtree[node_idx] = 1
                r_node[node_idx] = 0.0
                r_branch[node_idx] = 0.0
            else:
                # internal node => not a leaf in pruned tree (yet)
                leaves_in_subtree[node_idx] = 0
                candidate_nodes[node_idx] = 1
                n_leaves_subtree[node_idx] = 0
                r_node[node_idx] = 0.0
                # per-node gain contribution (weighted, normalized by root weight)
                r_branch[node_idx] = (
                    nodes[node_idx].improvement * weighted_n_node_samples[node_idx]
                ) / weighted_n_node_samples[0]

        # iterative postorder to accumulate n_leaves_subtree and total subtree gain
        sp = 0
        stack_nodes[sp] = 0
        stack_state[sp] = 0
        sp += 1
        ...
        while sp > 0:
            ...
            if stack_state[sp] == 0:
                ...
                left = children_left[cur]
                if left != _TREE_LEAF:
                    right = children_right[cur]
                    parent[left] = cur
                    parent[right] = cur

                    stack_nodes[sp] = right
                    stack_state[sp] = 0
                    sp += 1
                    stack_nodes[sp] = left
                    stack_state[sp] = 0
                ...
                if left != _TREE_LEAF:
                    right = children_right[cur]
                    n_leaves_subtree[cur] = n_leaves_subtree[left] + n_leaves_subtree[right]
                    r_branch[cur] += r_branch[left] + r_branch[right]
                else:
                    n_leaves_subtree[cur] = 1

        # record the unpruned tree (alpha = 0)
        ...
        while True:
            ...
            # find weakest link among current candidates
            for node_idx in range(n_nodes):
                if in_subtree[node_idx] and candidate_nodes[node_idx]:
                    n_leaves = n_leaves_subtree[node_idx]
                    if n_leaves <= 1:
                        continue
                    effective_alpha = r_branch[node_idx] / (n_leaves - 1)
                    if effective_alpha < alpha_min:
                        alpha_min = effective_alpha
                        best_node = node_idx
            ...
            # record pruning step
            controller.save_metrics(alpha_min, r_branch[0])

            # stop once next prune would reach/exceed requested ccp_alpha
            if controller.stop_pruning(alpha_min):
                break

            # prune subtree rooted at best_node: best_node becomes a leaf
            leaves_in_subtree[best_node] = 1

            n_leaves = n_leaves_subtree[best_node]
            r_diff = r_node[best_node] - r_branch[best_node]  # == -subtree_gain(best_node)

            # mark descendants out of subtree + clear any leaf markings there
            sp2 = 0
            left = children_left[best_node]
            if left != _TREE_LEAF:
                right = children_right[best_node]
                stack2[sp2] = right; sp2 += 1
                stack2[sp2] = left; sp2 += 1

            while sp2 > 0:
                sp2 -= 1
                cur = stack2[sp2]
                in_subtree[cur] = 0
                leaves_in_subtree[cur] = 0
                candidate_nodes[cur] = 0
                left = children_left[cur]
                if left != _TREE_LEAF:
                    right = children_right[cur]
                    stack2[sp2] = right; sp2 += 1
            ...
            candidate_nodes[best_node] = 0
            n_leaves_subtree[best_node] = 1
            r_branch[best_node] = r_node[best_node]  # == 0.0

            # update ancestors: remove (n_leaves-1) leaves and subtract subtree gain
            parent_idx = parent[best_node]
            while parent_idx != _TREE_UNDEFINED:
                n_leaves_subtree[parent_idx] -= (n_leaves - 1)
                r_branch[parent_idx] += r_diff
                parent_idx = parent[parent_idx]

        # controller expects the *leaf mask*
        controller.after_pruning(leaves_in_subtree)
...
cdef class _AlphaPrunerAUC(_CCPPruneController):
    """Use alpha to control when to stop pruning for AUC (gain-based) criterion.
    
    For AUC, effective_alpha represents gain per additional leaf (positive value).
    We prune subtrees with small gain first, stopping when we would prune
    subtrees that are too valuable.
    """
    cdef float64_t ccp_alpha
    cdef intp_t capacity
    ...
    cdef bint stop_pruning(self, float64_t effective_alpha) noexcept nogil:
        # For gain-based: stop when effective_alpha (gain per leaf) becomes too large
        # Continue pruning while effective_alpha <= ccp_alpha
        # Stop when effective_alpha > ccp_alpha (too valuable to prune)
        return effective_alpha >= self.ccp_alpha
...
def _build_pruned_tree_ccp_auc(
    Tree tree,  # OUT
    Tree orig_tree,
    float64_t ccp_alpha
):
    ...
    pruning_controller = _AlphaPrunerAUC(ccp_alpha=ccp_alpha)

    _cost_complexity_prune_auc(leaves_in_subtree, orig_tree, pruning_controller)
    ...
    with nogil:
        ...
        while not prune_stack.empty():
            ...
            new_node_id = tree._add_node(
                ...
                node.weighted_n_node_samples, node.improvement, node.missing_go_to_left)
...
def ccp_pruning_path_auc(Tree orig_tree):
    cdef uint8_t[:] leaves_in_subtree = np.zeros(shape=orig_tree.node_count, dtype=np.uint8)

    path_finder = _PathFinder(orig_tree.node_count)

    _cost_complexity_prune_auc(leaves_in_subtree, orig_tree, path_finder)

    cdef uint32_t total_items = path_finder.count
    cdef float64_t[:] ccp_alphas = np.empty(shape=total_items, dtype=np.float64)
    cdef float64_t[:] gains = np.empty(shape=total_items, dtype=np.float64)
    cdef uint32_t count = 0

    while count < total_items:
        ccp_alphas[count] = path_finder.ccp_alphas[count]
        gains[count] = path_finder.impurities[count]   # rename “impurities” -> “gains” on Python side
        count += 1

    return {'ccp_alphas': np.asarray(ccp_alphas), 'gains': np.asarray(gains)}
